...
class TableConfigurator {
  ...
  updateURL() {
    ...
    if (window.parent !== window) {
      window.parent.postMessage({ type: 'configurator-state', params: params.toString() }, '*');
    }
  }
  ...
  setupScene() {
    ...
    this.camera.position.set(4.0, 0.85, 5.5);
  ...
  setupLighting() {
    ...
    envScene.background = new THREE.Color(0xf5f4f0);
    // Add some gradient lights to the env scene for soft reflections
    const envLight1 = new THREE.RectAreaLight(0xffffff, 2, 10, 10);
  ...
  async loadModel(shapeId) {
    ...
    try {
      ...
      // Apply materials
      await this.applyTopMaterial(this.state.materialType, this.state.color);
  ...
  async applyTopMaterial(materialTypeId, colorId) {
    ...
    if (materialTypeId === 'ceramic') {
      ...
      // Yakisugi: use the texture itself as a bump map for rough charred surface
      if (colorDef.bumpScale) {
        const bumpTex = texture.clone();
        bumpTex.needsUpdate = true;
        topMatProps.bumpMap = bumpTex;
        topMatProps.bumpScale = colorDef.bumpScale;
      }
      ...
      if (colorDef.bumpScale) {
        ...
        edgeMatProps.bumpMap = edgeBump;
        edgeMatProps.bumpScale = colorDef.bumpScale * 1.2; // slightly more on edge
      }
  ...
  loadTexture(url) {
    ...
    return new Promise((resolve, reject) => {
      this.textureLoader.load(
        url,
        (tex) => { this.textureCache[url] = tex; resolve(tex); },
        undefined, reject
      );
    });
  }
  ...
  updateLoaderProgress(pct) {
    ...
    if (el) el.textContent = `Jouw Droomtafel wordt geladen... ${pct}%`;
  }
  ...
  renderLegGrid() {
    ...
    const allLegs = this.legObjects.map((leg, i) => ({...leg, index: i}))
      .filter(l => !excludeLegs.includes(l.displayName) && !ceramicExclude.includes(l.displayName) && !smallTableExclude.includes(l.displayName));
    ...
    const activeIsExcluded = (isCeramic && (this.legObjects[this.activeLegIndex]?.isWood || ceramicExclude.includes(activeLegName)))
      || smallTableExclude.includes(activeLegName);
    ...
    const getLegSwatch = (name, isWood) => {
      ...
      if (file) return `<img src="Swatches/Onderstel/${file}" alt="${name}"/>`;
  ...
  updateMaterialSectionIcon() {
    ...
    if (!color) return;
    const bg = color.file ? `background-image:url('${color.file}');background-size:cover;` : `background-color:${color.swatch};`;
