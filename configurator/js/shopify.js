...
const EDGE_VARIANTS = {
  ...
  '20graden':     53624398872909,
  '20graden-inv': 53624398905677,
  'facet-bol':    53624398938445,
...
const DIKTEBLAD_VARIANTS = {
  1.2: 53627463172429,  // 12mm - €0
  2:   53627463205197,  // 20mm - €150
};
...
export function getLineItems(state) {
  ...
  // 4b. Dikte blad (only for ceramic)
  if (state.materialType === 'ceramic') {
    const thickness = (state.topThickness && state.topThickness !== 4) ? state.topThickness : 1.2;
    const dikteVariantId = DIKTEBLAD_VARIANTS[thickness];
    if (dikteVariantId) {
      items.push({
        label: 'Dikte blad',
        variantId: dikteVariantId,
        price: getPrice(dikteVariantId)
      });
    }
  }
