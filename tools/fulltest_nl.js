...
(async () => {
  ...
  for (const shape of shapes) {
    ...
    for (let i = 0; i < cards.length; i++) {
      const r = await page.evaluate(async ([i, process_env_NL]) => {
        ...
        if (process_env_NL && !window.__L) { const m = await import('./js/locale.js?v=' + (document.querySelector('link[rel=modulepreload]').href.split('v=')[1])); window.__L = m.L; }
        const zwDisp = window.__L ? window.__L(c.state.zwLegName) : c.state.zwLegName;
        return { title, zw: zwDisp, zwRaw: c.state.zwLegName, leg: c._selectedVariants?.leg || null,
                 visible: c.legObjects.filter(l => l.object.visible).map(l => l.displayName) };
      }, [i, !!process.env.NL]);
      const exp = NEW_LEGS[r.title];
      if (process.env.NL) console.log(`  ok  ${r.title} -> ${r.zwRaw} variant=${r.leg} badge="${cards[i].price}" visible=${r.visible.join('|')}`);
